# ============================================================
# SYSTEM AUDIT REPORT — Pre-Migration Baseline Blueprint
...
│   ├── docker-compose.yaml           # ★ MAIN compose — ALL services
...
| Category | Count |
|----------|-------|
| Docker Compose files | 3 (`docker-compose.yaml`, `dograh-docker-compose.yaml`, `local-validation/backup-docker-compose.yaml`) |
| Dockerfiles | 8 (api, brain, memory, tasks, tools, trainer, voice, ui) |
...
| Shell scripts | 36 (scripts/ + scripts/vps/) |
| Python services | 7 (api, brain, memory, tasks, tools, trainer, voice) |
...
This is the **unified production compose** that runs ALL services in a single stack.
...
| ollama | ollama | 11434 | NONE (internal) | ollama/ollama:0.3.14 | — | Local LLM server |
| fazle-api | fazle-api | 8100 | 127.0.0.1:8100 | — | ./fazle-system/api | Fazle API gateway |
| fazle-brain | fazle-brain | 8200 | NONE (internal) | — | ./fazle-system/brain | AI reasoning engine |
...
| fazle-trainer | fazle-trainer | 8600 | NONE (internal) | — | ./fazle-system/trainer | Knowledge extraction |
...
**Total: 26 services (in main compose)**
...
|---------|--------|----------|----------|
| app-network | bridge | NO | livekit, coturn, dograh-api, dograh-ui, cloudflared, fazle-api, fazle-brain, fazle-memory, fazle-task-engine, fazle-web-intelligence, fazle-trainer, fazle-voice, fazle-ui, prometheus, grafana |
| db-network | bridge | YES | postgres, redis, minio, qdrant, livekit, dograh-api, fazle-api, fazle-brain, fazle-memory, fazle-task-engine |
| ai-network | bridge | YES | ollama, fazle-brain, fazle-memory, fazle-task-engine, fazle-web-intelligence, fazle-trainer, fazle-voice |
...
| **Single Redis Instance** | OK | 1 Redis, but used with different DB numbers: DB 0 (Dograh/LiveKit), DB 1 (Brain), DB 2 (Trainer). |
| **Single Ollama Instance** | OK | 1 Ollama, 6GB memory limit. |
...
Fazle Trainer (8600) ──▶ OpenAI or Ollama [extraction]
...
### 3.9 — External APIs Used

| API | Used By | Purpose |
|-----|---------|---------|
| OpenAI Chat Completions | Brain, Trainer | LLM reasoning |
...
| Ollama (local) | Brain, Trainer | Local LLM alternative |
...
| **Ollama** | LLM model storage | Downloaded models (llama3.1 etc.) |
...
```
 ...
 (text/voice)       │       │                                    │
                    ...
                    │   ├── Silero VAD                           │
                    │   ├── OpenAI Whisper (STT)                │
                    │   ├── Brain Chat (LLM via Brain service)  │
...
User → Fazle UI → NextAuth (JWT) → Fazle API Gateway → Safety Check →
  ...
  Query LLM (OpenAI/Ollama) → Extract Memory Updates →
  Store Memories (Qdrant) → Execute Actions → Response
```
...
|-----------|---------------|
| **Total Services** | 26 (in single docker-compose.yaml) |
| **Custom Services** | 8 (Fazle microservices, all Python 3.12 + FastAPI) |
...
| **Cache** | Redis 8.0, single instance, DB 0/1/2 |
| **Vector DB** | Qdrant v1.17, collection: fazle_memories |
| **LLM** | OpenAI GPT-4o (primary), Ollama llama3.1 (fallback) |
...
| **Docker Compose Files** | 2 production (main + dograh-standalone) + 1 backup |
...
*This report serves as the baseline blueprint for safely migrating the system to the new three-compose architecture without breaking any functionality.*
